...
export const ClientProductGrid = () => {
  ...
  const categories = ["Todos", ...Array.from(new Set(products.map((p) => p.category || "Otro")))];
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 pb-28 w-full">
            {filteredProducts.map((product) => {
              ...
              return (
                ...
                >
                  {/* Circular Image Area */}
                  <div className="flex-shrink-0 w-14 h-14 bg-slate-100 rounded-full flex items-center justify-center overflow-hidden relative border-2 border-white shadow-sm transition-transform duration-500 group-hover:scale-110">
                    {product.image ? (
